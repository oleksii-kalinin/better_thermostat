...
class BetterThermostat(ClimateEntity, RestoreEntity, ABC):
    ...
    def _compute_hvac_action(self):  # helper kept internal for clarity
        ...
        if action == HVACAction.IDLE:
            try:
                # Skip overrides while we intentionally ignore TRV states, when
                # window is open, or when temperature is at/above target
                # (tolerance_hold).  A TRV still physically heating due to
                # thermal lag must not override the decision to stop — doing so
                # corrupts attr_hvac_action and causes calibration modes (e.g.
                # HEATING_POWER) to keep pushing heat.
